            ...
            <form class="bg-white px-4 py-3 border-divider rounded-bottom-8 shadow-sm" method="POST" action="{{ route('purchase-return.store') }}" id="purchaseReturnForm">
               ...
               <div class="row">
                  ...
                  <div class="mb-3 col-md-2 other_invoice_div" style="display:none">
                     ...
                     <input type="number" class="form-control" id="original_invoice_value" name="other_invoice_value" placeholder="Including taxes" step="0.01">
                  </div>
                        ...
                        <table id="sundry_up_table" class="table-striped table m-0 shadow-sm table-bordered">
                           ...
                           <tbody class="testWrapper">
                              <tr id="billtr_1" class="font-14 font-heading bg-white bill_taxes_row sundry_tr">
                                 ...
                                 <td class="w-min-50 ">
                                    <input class="bill_amt w-100 form-control" type="number" name="bill_sundry_amount[]" id="bill_sundry_amount_1" data-id="1" readonly  style="text-align:right;" step="0.01">
                                 </td>
                                 <td>
...
<script>
    ...
    // Function to calculate amount and update total sum
      function calculateAmount(key=null) {  
        let customer_gstin = $('#party_id option:selected').data('gstin') || "";
         ...
         let gstBreakupHtml = '';
         customer_gstin_local =
            $("#party_id option:selected").data("gstin") || "";

         merchant_gstin_local =
            $("#series_no option:selected").data("gst_no")
            || merchant_gstin
            || "";

         let vendor_gstin =
            customer_gstin_local.substring(0,2);

         let company_gstin =
            merchant_gstin_local.substring(0,2);
  ...
  $(document).ready(function() {
   ...
   $(document).on('click',".unit",function(){
      ...
      let itemname = $("#goods_discription_tr_"+$(this).attr('data-id')+" option:selected").text();
      ...
      let id = $(this).attr('data-id');
      let item_id = $("#goods_discription_tr_"+id).val();
      ...
      $.ajax({
         ...
         data: {
            ...
            purchase_bill_id : $("#purchase_bill_id").val(),

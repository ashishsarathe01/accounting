                        ...
                        @if(count($sale_order_items)>0)
                           @foreach ($sale_order_items as $sale_order_item)
                              <tr id="tr_{{$add_more_count}}" class="font-14 font-heading bg-white">
                                 ...
                                 <td class="w-min-50">
                                    <input type="number" class="quantity w-100 form-control" id="quantity_tr_{{$add_more_count}}" name="qty[]" placeholder="Quantity" style="text-align:right;" data-id="{{$add_more_count}}" value="{{$sale_order_item['qty']}}" />
                                 </td>
                                 ...
                                 <td class="col-total-weight-cell">
                                    <input type="number" class="total_weight form-control" id="total_weight_tr_{{$add_more_count}}" name="total_weight[]" placeholder="Total Wt." style="text-align:right;" data-id="{{$add_more_count}}" readonly/>
                                 </td>
                                 <td class="w-min-50">                              
...
function applyDualUnitMode(rowId, isDual) {
    ...
    // Show/hide weight cells in this row
    if (!isDual) {
...
function generateWeightBoxes(rowId, qty) {
    ...
    let currentCount = $container.find('.weight-input').length;
    ...
    if (qty > currentCount) {
        for (let i = currentCount + 1; i <= qty; i++) {
            let $input = $('<input>', {
                ...
                placeholder: 'Wt ' + i,
                'data-row': rowId,
                style: 'width:70px;text-align:right;margin-bottom:2px;'
            });
            $container.append($input);
        }
    }

    // REMOVE boxes if qty decreased
    if (qty < currentCount) {
        $container.find('.weight-input').slice(qty).remove();
    }

    // Re-label placeholders
    $container.find('.weight-input').each(function (idx) {
...
function sumWeights(rowId) {
    ...
    let total = 0;
    $('#weight_boxes_' + rowId + ' .weight-input').each(function () {
...
// Weight input change → sum → recalculate
$(document).on('input', '.weight-input', function () {
    let rowId = $(this).data('row');
    sumWeights(rowId);
});

// Qty change → generate weight boxes if dual
...
// On item select → detect dual_unit

...
export class MerchantService {
  ...
  constructor(private http:HttpClient,private configuration: Configuration) {
   ...
   addMerchant(name: string, email: string,mobile: number,GSTIN: string,gst_name: string,address: string,state: string,pincode: string,login_user_id:string) {
      return this.http.post(this.url+'add-merchant', {name: name,email: email,mobile: mobile,GSTIN: GSTIN,gst_name: gst_name,address: address,state: state,pincode: pincode,created_by: login_user_id},this.con);
   }

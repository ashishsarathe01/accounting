...
<th class="col-total-weight border-none bg-light-pink text-body" style="text-align:right;padding-right:24px;min-width:80px;">Total Wt.</th>
                     ...
                     <tbody>
                        ...
                        @if(isset($SaleDescription) && count($SaleDescription) > 0)
                           @foreach ($SaleDescription as $sale_order_item)
                              @php
                                 $piece_weights = DB::table('taarobar_sale_description_piece_weights')
                                    ->where('sale_description_id', $sale_order_item->id)
                                    ->orderBy('piece_no')
                                    ->get();
                                    $piece_weights = collect($piece_weights);
                              @endphp
...
data-percent="{{$item_list->gst_rate}}" data-val="{{$item_list->unit}}" data-id="{{$item_list->id}}" data-itemid="{{$item_list->id}}" data-parameterized_stock_status="{{$item_list->parameterized_stock_status}}" data-config_status="{{$item_list->config_status}}" data-group_id="{{$item_list->group_id}}" data-dual_unit="{{$item_list->dual_unit ?? 0}}">{{$item_list->name}}</option>
                                    ...
                                    <div class="d-flex mb-1">
                                       <input type="text"
                                                ...
                                                value="{{ $sale_order_item->description ?? '' }}">
                                    </div>
                                 </div>
                                    @endif
                                 </td>
...
@endif
   ...
   <input type="number"
      ...
      id="total_weight_tr_{{$add_more_count}}"
      name="total_weight[]"
      placeholder="Total Wt."
      style="text-align:right;"
      data-id="{{$add_more_count}}"
      readonly
      value="{{ ($sale_order_item->dual_unit == 1)
         ? rtrim(rtrim(number_format((float)$sale_order_item->qty, 2, '.', ''), '0'), '.')
         : '' }}"/>
</td>
                                 <td class="w-min-50">
                                    <input type="text" class="w-100 form-control unit" id="unit_tr_{{$add_more_count}}" readonly style="text-align:center;" data-id="{{$add_more_count}}" value="{{$sale_order_item->s_name ?? ''}}"/>
                                    <input type="hidden" class="units w-100" name="units[]" id="units_tr_{{$add_more_count}}" value="{{$sale_order_item->unit ?? ''}}" />
                                 </td>

                                 <td class="w-min-50">
                                    <input type="number" class="price form-control" id="price_tr_{{$add_more_count}}" name="price[]" placeholder="Price" style="text-align:right;" data-id="{{$add_more_count}}" value="{{ rtrim(rtrim(number_format((float)$sale_order_item->price, 2, '.', ''), '0'), '.') }}" data-price="{{$sale_order_item->price}}"/>
                                 </td>
                                 <td class=""><input type="number" id="amount_tr_{{$add_more_count}}" class="amount w-100 form-control" name="amount[]" placeholder="Amount"  style="text-align:right;" data-id="{{$add_more_count}}" value="{{ isset($sale_order_item->amount) ? rtrim(rtrim(number_format((float)$sale_order_item->amount, 2, '.', ''), '0'), '.') : '' }}"/></td>
                                 <td class="row_action_btn">
...
@else
   ...
   <svg xmlns="http://www.w3.org/2000/svg"
      data-id="{{$add_more_count}}"
      class="bg-danger rounded-circle remove_row"
      width="24"
      height="24"
      viewBox="0 0 24 24"
      fill="none"
...
@endif
                        ...
                        @else
                           <tr id="tr_1" class="font-14 font-heading bg-white">
                              ...
                              @if($config && $config->show_description == 1)
                                <div class="description-wrapper mt-1" data-row="0">
                                    <div class="d-flex mb-1">
                                       <input type="text"
...
<td class="col-total-weight-cell">
   ...
   <?php } ?>
               ...
               <div class=" d-flex">
                  ...
                  <div class="ms-auto">
                     <input type="submit" value="UPDATE" class="btn btn-xs-primary" id="saveBtn">
         ...
         <div class="col-lg-1 d-none d-lg-flex justify-content-center px-1">
            <div class="shortcut-key w-100">
               ...
               <button class="p-2 transaction-shortcut-btn mb-2 d-flex align-items-center" data-bs-toggle="tooltip" data-bs-placement="bottom" title="Add Voucher">
                  <span class="border-bottom-black">F3</span><span class="ps-1 fw-normal text-body d-inline-block text-ellipsis">Add Voucher</span>
               </button>
               <button class="p-2 transaction-shortcut-btn mb-2 d-flex align-items-center" data-bs-toggle="tooltip" data-bs-placement="bottom" title="Add Payment">
                  <span class="border-bottom-black">F5</span><span class="ps-1 fw-normal text-body d-inline-block text-ellipsis">Add Payment</span>
               </button>
               <button class="p-2 transaction-shortcut-btn mb-2 d-flex align-items-center" data-bs-toggle="tooltip" data-bs-placement="bottom" title="Add Receipt">
                  <span class="border-bottom-black">F6</span><span class="ps-1 fw-normal text-body d-inline-block text-ellipsis">Add Receipt</span>
               </button>
               <button class="p-2 transaction-shortcut-btn mb-2 d-flex align-items-center" data-bs-toggle="tooltip" data-bs-placement="bottom" title="Add Journal">
                  <span class="border-bottom-black">F7</span><span class="ps-1 fw-normal text-body d-inline-block text-ellipsis">Add Journal</span>
               </button>
               <button class="p-2 transaction-shortcut-btn mb-2 d-flex align-items-center" data-bs-toggle="tooltip" data-bs-placement="bottom" title="Add Sales">
                  <span class="border-bottom-black">F8</span><span class="ps-1 fw-normal text-body d-inline-block text-ellipsis">Add Sales</span>
               </button>
               <button class="p-2 transaction-shortcut-btn mb-4 d-flex align-items-center " data-bs-toggle="tooltip" data-bs-placement="bottom" title="Add Purchase">
                  <span class="border-bottom-black">F9</span><span class="ps-1 fw-normal text-body d-inline-block text-ellipsis">Add Purchase</span>
               </button>
               <button class="p-2 transaction-shortcut-btn mb-2 d-flex align-items-center" data-bs-toggle="tooltip" data-bs-placement="bottom" title="Balance Sheet">
                  <span class="border-bottom-black">B</span><span class="ps-1 fw-normal text-body d-inline-block text-ellipsis">Balance Sheet</span>
               </button>
               <button class="p-2 transaction-shortcut-btn mb-2 d-flex align-items-center " data-bs-toggle="tooltip" data-bs-placement="bottom" title="Trial Balance">
                  <span class="border-bottom-black">T</span><span class="ps-1 fw-normal text-body d-inline-block text-ellipsis">Trial Balance</span>
               </button>
               <button class="p-2 transaction-shortcut-btn mb-2 d-flex align-items-center" data-bs-toggle="tooltip" data-bs-placement="bottom" title="Stock Status">
                  <span class="border-bottom-black">S</span><span class="ps-1 fw-normal text-body d-inline-block text-ellipsis">Stock Status</span>
               </button>
               <button class="p-2 transaction-shortcut-btn mb-2 d-flex align-items-center" data-bs-toggle="tooltip" data-bs-placement="bottom" title="Acc. Ledger">
                  <span class="border-bottom-black">L</span><span class="ps-1 fw-normal text-body d-inline-block text-ellipsis">Acc. Ledger</span>
               </button>
               <button class="p-2 transaction-shortcut-btn mb-2 d-flex align-items-center " data-bs-toggle="tooltip" data-bs-placement="bottom" title="Item Summary">
                  <span class="border-bottom-black">I</span><span class="ps-1 fw-normal text-body d-inline-block text-ellipsis">Item Summary</span>
               </button>
               <button class="p-2 transaction-shortcut-btn mb-2 d-flex align-items-center" data-bs-toggle="tooltip" data-bs-placement="bottom" title="Item Ledger">
                  <span class="border-bottom-black">D</span><span class="ps-1 fw-normal text-body d-inline-block text-ellipsis">Item Ledger</span>
               </button>
               <button class="p-2 transaction-shortcut-btn mb-2 d-flex align-items-center " data-bs-toggle="tooltip" data-bs-placement="bottom" title="GST Summary">
                  <span class="border-bottom-black">G</span><span class="ps-1 fw-normal text-body d-inline-block text-ellipsis">GST Summary</span>
               </button>
               <button class="p-2 transaction-shortcut-btn mb-2 d-flex align-items-center " data-bs-toggle="tooltip" data-bs-placement="bottom" title="Switch User">
                  <span class="border-bottom-black">U</span><span class="ps-1 fw-normal text-body d-inline-block text-ellipsis">Switch User</span>
               </button>
               <button class="p-2 transaction-shortcut-btn mb-2 d-flex align-items-center " data-bs-toggle="tooltip" data-bs-placement="bottom" title="Configuration">
                  <span class="border-bottom-black">F</span><span class="ps-1 fw-normal text-body d-inline-block text-ellipsis">Configuration</span>
               </button>
               <button class="p-2 transaction-shortcut-btn mb-2 d-flex align-items-center" data-bs-toggle="tooltip" data-bs-placement="bottom" title="Lock Program">
                  <span class="border-bottom-black">K</span><span class="ps-1 fw-normal text-body d-inline-block text-ellipsis">Lock Program</span>
               </button>
...
<script>
   ...
   var mat_series = "<?php echo !empty($GstSettings);?>";
   var bill_to_id = "{{$sale->party_id ?? $bill_to_id}}";
   var shipp_to_id = "{{$sale->shipping_name ?? $shipp_to_id}}";
   var enter_gst_status = 0;
   var auto_gst_calculation = 0;
   var customer_gstin = "";
   var merchant_gstin = "<?php echo !empty($mat_series) ? $mat_series[0]->gst_no : ''; ?>";
   var percent_arr = [];
   var add_more_count = {{$add_more_count}};
   var page_load = 0;
   var add_more_bill_sundry_up_count = {{$sundry_index ?? 2}};
...
$item_options .= '<option value="'.$item_data->id.'"
data-unit_id="'.$item_data->u_name.'"
data-percent="'.$item_data->gst_rate.'"
data-val="'.$item_data->unit.'"
...
'</td>' +
      ...
      $('#item_id_' + add_more_count).select2({
    width: '100%'
});
   }
...
function removeItem() {
  ...
  $(document).on("click", ".remove", function () {
    ...
    if (savedSizes && itemId) {
      try {
        let sizeObjs = JSON.parse(savedSizes);
        let sizeIds = sizeObjs.map(s => s.id);
...
function applyDualUnitMode(rowId, isDual) {

    $('#dual_unit_' + rowId).val(isDual ? 1 : 0);

    let $row = $('#tr_' + rowId);

    if (!isDual) {
    $('#weight_boxes_' + rowId).empty();
    $('#total_weight_tr_' + rowId).val('');
}
}

function generateWeightBoxes(rowId, qty) {

    let $container = $('#weight_boxes_' + rowId);
...
function sumWeights(rowId) {

    let total = 0;
    $('#weight_boxes_' + rowId + ' .piece_weight').each(function () {
        let v = parseFloat($(this).val()) || 0;
        total += v;
    });
    total = parseFloat(formatDecimal(total));
    $('#total_weight_tr_' + rowId).val(total > 0 ? total : '');
    calculateAmount();
}
...
$(document).on('input', '.quantity', function () {
    ...
    if (parseInt($('#dual_unit_' + rowId).val()) === 1) {
        let qty = parseInt($(this).val()) || 0;
        generateWeightBoxes(rowId, qty);
    }
});
...
$(document).on('change', '.item_id', function () {
    ...
    let dualUnit = parseInt(selected.data('dual_unit')) || 0;
    applyDualUnitMode(rowId, dualUnit === 1);
});
...
let under_group = $('#party option:selected').attr('data-under_group');
        ...
        $('#example11 tbody tr').each(function() {
            ...
            var rowId = $(this).find('.item_id').data('id');
            var isDual = rowId ? (parseInt($('#dual_unit_' + rowId).val()) === 1) : false;
            ...
            if(key=="A"){
               ...
               var amount = 0;
               if (isDual) {
                  var totalWt = parseFloat($('#total_weight_tr_' + rowId).val()) || 0;
                  amount = (price && totalWt) ? (price * totalWt) : 0;
               } else {
                  amount = (price && quantity) ? (price * quantity) : 0;
               }
         ...
         if(customer_gstin==merchant_gstin.substring(0,2)){
            ...
            if(result.length>0){
               ...
               result.forEach(function(e){
                  item_total_amount = parseFloat(item_total_amount) + parseFloat(e.amount);
               });
               result.forEach(function(e,i){
                  ...
                  let taxable_amount_per_item = (e.amount/item_total_amount)*(bill_sundry_total);
                  taxable_amount_per_item = parseFloat(e.amount) + parseFloat(taxable_amount_per_item);
                  if(index==1){
                     if(enter_gst_status==0 && item_taxable_amount!=0 && auto_gst_calculation==1){
                        let sundry_amount = (taxable_amount_per_item*e.percent/2)/100;
            ...
            if(result.length>0){
               ...
               result.forEach(function(e,i){
                  ...
                  if(index==1){
                     if(enter_gst_status==0 && item_taxable_amount!=0){
                        let sundry_amount = (taxable_amount_per_item*e.percent)/100;
    ...
    // Re-enforce after DOM settles
    setTimeout(function(){
        let addrToSet = (bill_to_address_id != "" && bill_to_address_id != "0" && bill_to_address_id != "null")
                        ? bill_to_address_id
                        : 'main';

        if($("#address option[value='" + addrToSet + "']").length){
            $("#address").val(addrToSet);
            let gstin = $("#party option:selected").attr('data-gstin') || '';
            if(addrToSet === 'main'){
                let mainAddr = $("#party option:selected").attr('data-address') || '';
                $("#partyaddress").html("GSTIN : " + gstin + "<br>Address : " + mainAddr);
            } else {
                let selOpt  = $("#address option[value='" + addrToSet + "']");
                let addr    = selOpt.attr('data-address') || '';
                let pincode = selOpt.attr('data-pincode') || '';
                $("#partyaddress").html("GSTIN : " + gstin + "<br>Address : " + addr + ", " + pincode);
            }
        }
    }, 200);
...
$(document).on('change', '#party', function(){      
    ...
    // Manually update the display WITHOUT relying on trigger('change')
    // so it works even before select2 is initialized
    let gstin    = $('option:selected', '#party').attr('data-gstin') || '';
    if(addrToSet === 'main' || addrToSet === ''){
        let mainAddr = $('option:selected', '#party').attr('data-address') || '';
        $("#partyaddress").html("GSTIN : " + gstin + "<br>Address : " + mainAddr);
    } else {
        let selOpt  = $("#address option[value='" + addrToSet + "']");
        let addr    = selOpt.attr('data-address') || '';
...
$("#address").change(function(){
    ...
    if(val !== "" && val !== "main"){
        ...
        $("#partyaddress").html("GSTIN : " + gstin + "<br>Address : " + address + ", " + pincode);
    } else {
        $("#partyaddress").html("GSTIN : " + gstin + "<br>Address : " + mainAddr);
    }
});
...
$(document).on("keydown", ".amount", function (event) {
  if (event.key === "Enter") {
    ...
    let $current = $(this);
    let id = parseInt($current.data("id"));
...
function checkVoucherDuplicate() {
        ...
        $.ajax({
            ...
            success: function(response) {
                if (response.exists) {
                    alert('This voucher number already exists! Please generate or assign another.');
                    $('#voucher_prefix').val('');
                    $('#voucher_no').val('');
                }
            },
...
option.setAttribute('data-unit_id', res.item.unit ?? '');
option.setAttribute('data-unit_name', res.item.u_name ?? '');
option.setAttribute('data-percent', res.item.gst_rate ?? 0);
...
option.setAttribute('data-dual_unit', res.item.dual_unit ?? 0);
...
$(document).on('click','.remove_row',function(){
    ...
    $('#example11 tbody tr[id^="tr_"]').each(function(index){
...
// DUAL UNIT: item change -> show/hide weight columns
$(document).on('change', '.item_id', function () {
...
$(document).on('input', '.piece_weight', function () {
    ...
    total = parseFloat(formatDecimal(total));
    $('#total_weight_tr_' + row_id).val(total > 0 ? total : '');

    calculateRowAmount(row_id);
});

// price change -> recalculate row
$(document).on('input', '.price', function () {
    let row_id = $(this).data('id');
    if (row_id) {
        calculateRowAmount(row_id);
...
// calculateRowAmount: safe amount calc + calls calculateAmount
function calculateRowAmount(row_id) {

    if (!row_id) return;

    let dual_unit = parseInt($('#dual_unit_' + row_id).val()) || 0;
    let qty       = parseFloat($('#quantity_tr_' + row_id).val()) || 0;
    let tw        = parseFloat($('#total_weight_tr_' + row_id).val()) || 0;
    let price     = parseFloat($('#price_tr_' + row_id).val()) || 0;
    ...
    $('#amount_tr_' + row_id).val(parseFloat(amount).toFixed(2));
    calculateAmount();
}

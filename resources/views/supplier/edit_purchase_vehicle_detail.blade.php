    ...
    <section class="list-of-view-company-section container-fluid">
        <div class="row vh-100">
            ...
            <div class="col-md-12 ml-sm-auto  col-lg-10 px-md-4 bg-mint">
                ...
                <form class="bg-white px-4 py-3 border-divider rounded-bottom-8 shadow-sm" method="POST" action="{{ route('update-purchase-info',$purchase_info->id) }}">
                    @csrf
                    <div class="row">
                        <div class="mb-3 col-md-3">
                            <label for="vehicle_bo" class="form-label font-14 font-heading">Vehicle No.</label>
                            <input type="text" class="form-control"  placeholder="Enter Vehicle No." id="vehicle_no" name="vehicle_no" required value="{{$purchase_info->vehicle_no}}">
                        </div>
                        ...
                        <div class="mb-3 col-md-3">
                            ...
                            <input type="text" class="form-control"  placeholder="Enter Gross Weight." id="gross_weight" name="gross_weight" required value="{{$purchase_info->gross_weight}}">
                        </div>
                        <div class="clearfix"></div>
                        <div class="mb-4 col-md-4">
                            ...
                            <select class="form-select select2-single" id="account" name="account" required>
                                ...
                                @foreach($accounts as $key => $account)
                                    <option value="{{$account->id}}" @if($purchase_info->account_id==$account->id) selected @endif>{{$account->account_name}}</option>
                                @endforeach                                
                            </select>
                        </div>
                    </div>

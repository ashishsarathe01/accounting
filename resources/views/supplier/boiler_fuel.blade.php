...
@if($status === null)
    <script>
        document.addEventListener('DOMContentLoaded', function () {
            fetch("{{ url('user-boiler-fuel-status') }}")
                .then(res => res.json())
                .then(data => {
                    if (data.status !== undefined) {
                        window.location.href = "{{ url('boiler-fuel') }}?status=" + data.status;
                    }
                });
        });
    </script>
                ...
                <div class="tab-content pt-0" id="tab-content">
                    ...
                    @can('view-module', 111)
                        ...
                        <div class="d-flex align-items-center gap-3">
                                ...
                                <select class="form-select form-select-sm"
                                        style="width:220px"
                                        onchange="window.location='{{ url('boiler-fuel') }}?status=' + this.value" id="statusFilter">
                                     @can('view-module', 111)
                            ...
                            <h5 class="transaction-table-title m-0" id="openMissingModal">
                                BOILER FUEL <span id="missing">({{ $count }})</span>
                            </h5>
                             @can('view-module', 100)
                    ...
                    @can('view-module', 112)
                        ...
                        <div class="d-flex align-items-center gap-3">
                            ...
                            <select class="form-select form-select-sm"
                                    style="width:220px"
                                    onchange="window.location='{{ url('boiler-fuel') }}?status=' + this.value">
                                 @can('view-module', 111)
                                                <option value="0" {{ $status == 0 ? 'selected' : '' }}>
                                                    Pending, In Process
                                                </option>
              ...
              <div class="modal-dialog modal-dialog-centered modal-lg">
                <div class="modal-content">
                  <div class="modal-header bg-dark text-white">
                    <h5 class="modal-title text-white" id="missingModalLabel">Missing Entries</h5>
                  ...
                  <div class="modal-body">
                          ...
                          <table class="table table-bordered table-striped">
                              <thead class="table-dark text-white">
                                  <tr>
                                      ...
                                      <th class="text-white">Missing Value</th>
                                  </tr>
                              </thead>
                              <tbody>
                      ...
                      @else
                          <p class="text-muted text-center text-white mb-0">No missing values found.</p>
                      @endif
                  </div>
...
<script>
                    ...
                    success:function(res){
                        ...
                        if(res!=""){
                            ...
                            if(map_purchase_id!="" && map_purchase_id!=0){
                                ...
                                let qty = 0; let rate = 0;
                                $(".qty").each(function(){                                    
                                    let id = $(this).attr('data-id');                                    
                                    if(id!="cut" && id!="short_weight" && $(this).val()!="" && $(this).val()!=0){
                                        qty = $(this).val();  
                                        rate = $("#contract_rate_"+id).val(); 
                                    }                                
                                });
    ...
    $(".calculate").keyup(function(){
        ...
        let diff_rate = parseFloat(bill_rate) - parseFloat(contract_rate);
            ...
            success:function(res){
                ...
                if(response.status == true){
                    ...
                    if(approve_from_date!="" && approve_to_date!=""){
                        window.location = "boiler-fuel?status="+page_status
                ...
                success:function(res){
                    ...
                    if(res!=""){
                        if(res.item_price){
                            ...
                            let amt = qty*contract_rate;
                            amt = amt.toFixed(2);
                            $("#report_amount_cut").val(amt);
                            //Short Weight Calculation
                            $("#contract_rate_short_weight").val(0);                            
                            var short_weight_qty = $("#qty_short_weight").val();
                            var short_weight_bill_rate = $("#bill_rate_short_weight").val();
                            var contract_rate_short_weight = $("#contract_rate_short_weight").val();
                            if(short_weight_qty == ''){
                                short_weight_qty = 0;
                            }
                            if(short_weight_bill_rate == ''){
                                short_weight_bill_rate = 0;
                            }
                            if(contract_rate_short_weight == ''){
                                contract_rate_short_weight = 0;
                            }
                            let short_weight_diff_rate = short_weight_bill_rate - contract_rate_short_weight;
                            var short_weight_difference_amount = parseFloat(short_weight_qty) * parseFloat(short_weight_diff_rate);
                            $("#difference_amount_short_weight").val(short_weight_difference_amount.toFixed(2));
                            let amt1 = short_weight_qty*contract_rate_short_weight;
                            amt1 = amt1.toFixed(2);
                            $("#report_amount_short_weight").val(amt1);
    ...
    document.addEventListener('DOMContentLoaded', function () {
        ...
        // Check if the element exists before attempting to set the value
        if (!statusFilter) {
        ...
        fetch("{{ url('user-boiler-fuel-status') }}")
            ...
            .then(data => {
                ...
                if (select.querySelector(`option[value="${status}"]`)) {
                    select.value = status;
                    window.location = "{{ url('boiler-fuel') }}?status=" + status;
                }
            });
    });

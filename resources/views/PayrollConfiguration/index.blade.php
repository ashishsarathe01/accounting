    ...
    <section class="list-of-view-company-section container-fluid">
        <div class="row vh-100">
            ...
            <div class="col-md-12 ml-sm-auto col-lg-9 px-md-4 bg-mint">
                ...
                <form method="POST" action="{{ route('payroll.configuration.store') }}" class="px-4 py-4">
                    @csrf
                    <div class="row">
                        <div class="col-md-4 mb-3">
                            <label class="form-label fw-semibold">
                                TDS Applicable Account
                            </label>
                            <select class="form-select select2" name="tds_account_id">
                                <option value="">-- Select TDS Account --</option>
                                @foreach($statutory_account_list as $account)
                                    <option value="{{ $account->id }}"
                                        {{ (!empty($tds_account_id) && $tds_account_id == $account->id) ? 'selected' : '' }}>
                                        {{ $account->name }}
                                    </option>
                                @endforeach
                            </select>
                        ...
                        <div class="col-md-4 mb-3">
                            <label class="form-label fw-semibold">
                                PF Applicable Account
                            </label>
                            <select class="form-select select2" name="pf_account_id">
                                <option value="">-- Select PF Account --</option>
                                @foreach($statutory_account_list as $account)
                                    <option value="{{ $account->id }}"
                                        {{ (!empty($pf_account_id) && $pf_account_id == $account->id) ? 'selected' : '' }}>
                                        {{ $account->name }}
                                    </option>
                                @endforeach
                            </select>

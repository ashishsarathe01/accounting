        ...
        <div class="row vh-100">
            ...
            <div class="col-md-12 ml-sm-auto col-lg-9 px-md-4 bg-mint">
                ...
                <h5 class="table-title-bottom-line px-4 py-3 m-0 bg-plum-viloet position-relative title-border-redius border-divider shadow-sm">
                    Payroll – PF Contribution Sheet
        <small class="text-muted">({{ $monthYear }})</small>
                </h5>
    <form method="GET" action="{{ url('/payroll-pf-sheet') }}" class="mb-3 d-flex align-items-center gap-2">
        ...
        <table class="table table-bordered table-sm text-center">
            ...
            @forelse($pfData as $row)
                <tr>
                    ...
                    <td class="text-end">{{ number_format($row['epf_wages'], 2) }}</td>
                    <td class="text-end">{{ number_format($row['eps_wages'], 2) }}</td>
                    <td class="text-end">{{ number_format($row['edli_wages'], 2) }}</td>

                    <td class="text-end">{{ number_format($row['epf_ee'], 2) }}</td>
                    <td class="text-end">{{ number_format($row['eps_er'], 2) }}</td>
                    <td class="text-end">{{ number_format($row['diff_er'], 2) }}</td>
            ...
            @empty
                <tr>
                    <td colspan="12">No PF data found for selected month</td>
                </tr>
            @endforelse
            </tbody>
        </table>

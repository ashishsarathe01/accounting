        ...
        <div class="row vh-100">
            ...
            <div class="col-md-12 ml-sm-auto  col-lg-10 px-md-4 bg-mint">
                ...
                <div class="table-title-bottom-line position-relative d-flex justify-content-between align-items-center bg-plum-viloet title-border-redius border-divider shadow-sm py-2 px-4">
                    <h5 class="transaction-table-title m-0 py-2">Manage Supplier Rate — Waste Kraft1</h5>
                    @can('view-module', 107)
                    ...
                    <form class="bg-white px-4 py-3 border-divider rounded-bottom-8 shadow-sm" method="POST" action="{{ route('store-supplier-rate') }}">
                        ...
                        <div class="row">
                            ...
                            <div class="mb-9 col-md-9">
                                @if(!empty($advance_rate) && count($advance_rate)>0)
                                    <p class="blink">Already Set Rate</p>
                                    @foreach($advance_rate as $key => $value)
                                        <a href="{{ route('manage-supplier-rate') }}/waste-kraft/{{$value}}"><button type="button" class="btn btn-info">{{date('d-m-Y',strtotime($value))}}</button></a>
                                    @endforeach
                                @endif
                            </div>
                                ...
                                @foreach($locations as $key => $location)
                                    ...
                                    @if(!empty($heads) && count($heads)>0)
                                        @foreach($heads as $k => $head)
                                            @if($head->group_type=="WASTE KRAFT")
                                                @php
                                                    $array_id = $location->id . "_" . $head->id;
                                                ...
                                                <div class="mb-{{$col}} col-md-{{$col}}">
                                                    @if($key==0)<label for="name" class="form-label font-14 font-heading ">{{$head->name}}</label>@endif
                                                    <input type="hidden" name="head_id_{{$location->id}}[]" value="{{$head->id}}">
                                                    <input type="number" step="any" class="form-control @if($k==0)first_rate @else other_rate_{{$location->id}} @endif" name="head_value_{{$location->id}}[]" data-location_id="{{$location->id}}" data-head_id="{{$head->id}}" placeholder="Enter {{$head->name}} RATE" required @cannot('view-module', 208) readonly @endcannot value="<?php if(isset($supplier_rates) && isset($supplier_rates[$array_id]) && count($supplier_rates)>0){ echo $supplier_rates[$array_id]; }?>">
                                                </div>
                                            @endif
                                        @endforeach
                                    @endif
                ...
                @can('view-module', 209)
                <!-- View Supplier Rate Table -->
                <h5 class="table-title-bottom-line px-4 py-3 mt-4 m-0 bg-plum-viloet position-relative title-border-redius border-divider shadow-sm">
                    View Supplier Rate
                </h5>
                <div class="bg-white table-view shadow-sm mt-3">
                    <table class="table table-striped table m-0 shadow-sm">
                        ...
                        <tbody>
                            @if(!empty($all_rate) && count($all_rate)>0)
                                @foreach($all_rate as $date_key => $rows)
                                    @php
                                        $ressult = [];
                                        foreach ($rows as $row) {
                            ...
                            @else
                                <tr>
                                    <td colspan="2" class="text-center">No rates found.</td>
                                </tr>
                            @endif
                        </tbody>
                    </table>
      ...
      <div class="modal-content p-4 border-divider border-radius-8">
        ...
        <div class="row">
            <div class="mb-2 col-md-2">
                <label class="form-label font-14 font-heading"><strong>Head Name</strong></label>
            </div>
            <div class="mb-2 col-md-2">
                <label class="form-label font-14 font-heading"><strong>Difference Rate</strong></label>
            </div>
            <div class="mb-2 col-md-2">
                <label class="form-label font-14 font-heading"><strong>Action</strong></label>
            </div>
            <div class="clearfix"></div>
...
<div class="modal fade" id="location_modal" tabindex="-1" aria-labelledby="exampleModalLabel" aria-hidden="true">
   <div class="modal-dialog modal-lg">
      <div class="modal-content p-4 border-divider border-radius-8">
        ...
        <div class="row">
            <div class="mb-4 col-md-4">
                <label class="form-label font-14 font-heading"><strong>Location Name</strong></label>
                <input type="text" class="form-control" placeholder="Enter Location" id="location_name">
                <input type="hidden" class="form-control" id="location_edit_id">
            </div>
            <div class="mb-4 col-md-4">
                <label class="form-label font-14 font-heading"><strong>Status</strong></label>
...
$(document).ready(function(){
    ...
    // auto fill other rates based on first_rate + difference
    ...
    // date change navigates (keeps original behavior)
    $("#date").change(function(){
        window.location = "{{ url('manage-supplier-rate') }}/waste-kraft/" + $(this).val();
    });
            ...
            success:function(res){
                if(res!=""){
                    ...
                    if(res.location.length>0){
                        ...
                        $(".location_tbl tbody").html('<tr><td colspan="2" class="text-center">No locations found.</td></tr>');
                    }
                }
            }
        });
            ...
            success:function(res){
                if(res!=""){
                    ...
                    if(res.bonus.length>0){
                        ...
                        const groupedByCategory = Object.groupBy ? Object.groupBy(res.bonus, product => product.account_name) : groupByFallback(res.bonus, 'account_name');
    ...
    // fallback groupBy for older browsers
    function groupByFallback(arr, key) {
...
// preserve active tab behavior (not used here, but harmless)
document.addEventListener('DOMContentLoaded', function () {
    // Restore the last active tab from localStorage (if any)

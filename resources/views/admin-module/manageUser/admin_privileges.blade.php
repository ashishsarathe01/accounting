        ...
        <div class="row vh-100">
            ...
            <div class="col-md-12 ml-sm-auto col-lg-10 px-md-4 bg-mint">
                ...
                <div class="d-flex justify-content-between align-items-center table-title-bottom-line px-4 py-3 m-0 bg-plum-viloet position-relative title-border-redius border-divider shadow-sm">
                    <h5 class="m-0">Set Admin Panel Privileges for: {{ $user->name }}</h5>
                    <div>
                        <a href="{{ route('admin.manageUser.assignCompanies', $user->id) }}" class="btn btn-xs-primary">
                            Assign Companies
                        </a>
                        <a href="{{ route('admin.manageUser.privileges', $user->id) }}" class="btn btn-xs-primary">
                            Set User Privileges
                        </a>
                    </div>
                </div>
                ...
                <div class="bg-white px-4 py-3 border-divider rounded-bottom-8 shadow-sm mt-0">
                    <form method="POST" action="{{ route('admin.manageUser.saveAdminPrivileges', $user->id) }}">
                        @csrf

                        <ul class="privilege-tree">
                            @foreach($privileges as $module)
                                @include('admin-module.manageUser.admin_privilege_module', ['module'=>$module, 'assigned'=>$assigned])
                            @endforeach
                        </ul>

                        <div class="text-center mt-4">
                            <button type="submit" class="btn btn-primary">Save Admin Panel Privileges</button>
                        </div>
                    </form>
                </div>
                <!-- End box container -->

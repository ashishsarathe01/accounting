      ...
      if(count($ledger)>0){
         foreach ($ledger as $key => $value) {
            ...
            if($value->entry_type==1){
               ...
               $action = SalesReturn::select('invoice_no','series_no','financial_year')->where('id',$value->entry_type_id)->first();
               $ledger[$key]->bill_no = "";
               if($action){
                  $ledger[$key]->bill_no = $action->series_no."/".$action->financial_year."/CR".$action->invoice_no;
               }               
               ...
               $action = Payment::select('voucher_no')->where('id',$value->entry_type_id)->first();
               $ledger[$key]->bill_no = $action->voucher_no;
            }else if($value->entry_type==6){
               $action = Receipt::select('voucher_no')->where('id',$value->entry_type_id)->first();
               $ledger[$key]->bill_no = $action->voucher_no;
            }else if($value->entry_type==7){
               $action = Journal::select('voucher_no')->where('id',$value->entry_type_id)->first();
               $ledger[$key]->bill_no = $action->voucher_no;
            }else if($value->entry_type==8){
               $action = Contra::select('voucher_no')->where('id',$value->entry_type_id)->first();
               $ledger[$key]->bill_no = $action->voucher_no;
            }else{               
               $ledger[$key]->bill_no = '';
            }            

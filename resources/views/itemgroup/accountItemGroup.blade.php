      ...
      <div class="row vh-100">
         ...
         <div class="col-md-12 ml-sm-auto  col-lg-10 px-md-4 bg-mint">
            ...
            <div class="position-relative table-title-bottom-line d-flex justify-content-between align-items-center bg-plum-viloet title-border-redius border-divider shadow-sm py-2 px-4">
               <h5 class="table-title m-0 py-2">List of Manage Group</h5>
               @can('action-module',78)
                  <a href="{{ route('account-item-group.create') }}" class="btn btn-xs-primary">
                  ADD
               ...
               <table id="example" class="table-striped table m-0 shadow-sm">
                  <thead>
                     <tr class=" font-12 text-body bg-light-pink ">
                        ...
                        <th class="w-min-120 border-none bg-light-pink text-body">Parameterized Stock Status</th>
                        ...
                        <tr class="font-14 font-heading bg-white">
                           ...
                           <td class="w-min-120">
                              <span class="bg-secondary-opacity-16 border-radius-4 text-secondary py-1 px-2 fw-bold">
                                 <?php
                                 if($value->parameterized_stock_status == 1){
                                    echo 'Enable';
                                 }else{
                                    echo 'Disable';
                                 }?>
                           ...
                           <td class="w-min-120 text-center">
                              @can('action-module',49)
                                 <a href="{{ URL::to('account-item-group/' . $value->id . '/edit') }}"><img src="{{ URL::asset('public/assets/imgs/edit-icon.svg')}}" class="px-1" alt=""></a>
                              @endcan
                              @can('action-module',50)
...
<div class="modal fade" id="delete_heading" tabindex="-1" aria-labelledby="exampleModalLabel" aria-hidden="true">
   <div class="modal-dialog w-360  modal-dialog-centered  ">
      <div class="modal-content p-4 border-divider border-radius-8">
         ...
         <form class="" method="POST" action="{{ route('account-item-group.delete') }}">
            @csrf

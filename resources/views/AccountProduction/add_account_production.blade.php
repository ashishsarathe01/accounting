   ...
   <section class="list-of-view-company-section container-fluid">
      <div class="row vh-100">
         ...
         <div class="col-md-12 ml-sm-auto  col-lg-9 px-md-4 bg-mint">
            ...
            <h5 class="table-title-bottom-line px-4 py-3 m-0 bg-plum-viloet position-relative title-border-redius border-divider shadow-sm">Manage Production</h5>
            <form id="frm" class="bg-white px-4 py-3 border-divider rounded-bottom-8 shadow-sm" method="POST" action="{{ route('account-production.store') }}">
               @csrf
               <div class="row">
                  <div class="mb-3 col-md-3">
                     ...
                     <select id="series_no" name="series_no" class="form-select" required >
                        ...
                        @foreach($series_list as $key => $value)
                           <option value="{{$value->series}}" data-invoice_start_from="" data-invoice_prefix="" data-manual_enter_invoice_no="1" data-duplicate_voucher="" data-blank_voucher=""  data-voucher_no="{{$value->voucher_no}}" @if(count($series_list)==1) selected  @endif>{{ $value->series }}</option>
                        @endforeach
                     </select>
                  </div>
                    <div class="mb-3 col-md-3">
               ...
               <div class="transaction-table transaction-main-table bg-white table-view shadow-sm border-radius-8 mb-4">
                  <table id="example11" class="table-striped table m-0 shadow-sm table-bordered">
                     ...
                     <tbody>
                        <tr id="tr_1" class="font-14 font-heading bg-white consumed_row">
                           ...
                           <td class="w-min-50">
                              <select class="form-select consume_item select2-single" name="consume_item[]" data-id="1" data-new="0" id="consume_item_1">
                ...
                <tbody>
                    ...
                    <!--<td>-->
                    <!--    <input type="number" class="form-control text-end" id="electricity_meter" name="electricity_meter" />-->
                    <!--</td>-->
                    <td>
                        <input type="number" class="form-control text-end" id="electricity_consumed_units" name="electricity_consumed_units" />
                    </td>
                    <td>
                        <input type="number" class="form-control text-end" id="electricity_consumed_units_night" name="electricity_consumed_units_night" />
                    </td>
                    <td>
                    ...
                    <td>2</td>
                    <td colspan="2"><input type="text" class="form-control" value="Fixed Cost" readonly /></td>
                    <td colspan="3"></td>
                    <td>
                ...
                <tbody id="generated_items_table">
                  ...
                  @php $rowId = 0; @endphp
                  @foreach($item_stock_size as $index1 => $item)
                     ...
                     <tr class="font-14 font-heading bg-white">
                        ...
                        <td>
                            <input type="text"
                                ...
                                value="{{ $item['name'] }}"
                                readonly>
                                <input type="hidden" name="generated_item_id[]" value="{{ $item['item_id'] }}" id="generated_item_id_{{ $rowId }}">
                        </td>
...
<script>
   ...
   $(document).on("click", ".add_more", function () {
      ...
      <td class="">
         <select class="form-select consume_item select2-single" name="consume_item[]" data-id="${add_more_count}" data-new="1" id="consume_item_${add_more_count}">
            ${optionElements}
         </select>
      </td>
      <td class="">
   ...
   function calculateAmountNew(id) {
      ...
      if(totalWeight > 0) {
         ...
         electricity_fixed_average = electricity_fixed_average.toFixed(8);
         let consume_item_price_arr = [];
            $(".consume_item").each(function(){
             if($(this).val()!=""){
                    ...
                    if(generatedItemId==generated_item_id){
                        items.forEach(item => {
                            if(consume_item_price_arr[item.consumed_item_id] && consume_item_price_arr[item.consumed_item_id]!='' && consume_item_price_arr[item.consumed_item_id]!=undefined){
                                generated_item_amount = parseFloat(generated_item_amount) + (item.quantity*consume_item_price_arr[item.consumed_item_id]);
   ...
   let consumptionReady = false;
   function calculateElectricity() {
       //if (!consumptionReady) return;
      //let previous = parseFloat($("#previous_units").val()) || 0;
      //let current = parseFloat($("#electricity_meter").val()) || 0;
      let consumed = parseFloat($("#electricity_consumed_units").val()) || 0;
      let consumed_night = parseFloat($("#electricity_consumed_units_night").val()) || 0;
      consumed = parseFloat(consumed) + parseFloat(consumed_night)
    ...
    //   }
     ...
     ///$("#electricity_consumed_units").val(consumed);
      ...
      calculateAmountNew(1); // Recalculate price per unit for generated items
   }
   ...
   $("#series_no").change(function(){
      ...
      let voucher_no = $('option:selected', this).attr('data-voucher_no');
      $("#voucher_prefix").val(voucher_no);
   ...
   $(document).on('change', '.consume_item', function(){
      ...
      if($(this).attr('data-new')==1 && $(this).val()!=""){
         alert("This is new item, please enter price and weight manually or set price and weight in set item.");
      }
      if($(this).val()==""){
   ...
   $(document).ready(function () {
      ...
      function calculateElectricity1() {
        // let previous = parseFloat($("#previous_units").val()) || 0;
         //let current = parseFloat($("#electricity_meter").val()) || 0;
         let consumed = parseFloat($("#electricity_consumed_units").val()) || 0;
         let price = parseFloat($("#electricity_unit_price").val()) || 0;
         let fixed = parseFloat($("#fixed_cost").val()) || 0;
         let amount = 0, itemsTotal = 0;
         // Calculate consumed units and amount
        //  if (current > previous) {
        //     consumed = current - previous;
        //  }
      ...
      // Trigger recalculation on any relevant change
      $(document).on("keyup change", "#electricity_consumed_units, #electricity_consumed_units_night, #electricity_unit_price, #fixed_cost", function () {
         calculateElectricity();
      });
      // Also recalc when items consumed table updates
      $(document).on("keyup change", ".consume_price, .consume_weight", function () {
      ...
      $.when.apply($, allRequests).done(function () {
            ...
            if(itemData.consumptionItems){
               itemData.consumptionItems.forEach(ci => {
                  ...
                  qty = parseFloat(qty.toFixed(3));
                    generated_items_quantity.push({"generated_item_id":itemData.generatedItemId,'consumed_item_id':ci.item_id,"quantity":qty});
         ...
         let staticRow = `
             <tr id="tr_${staticIndex}" class="font-14 font-heading bg-white consumed_row">
                  ...
                  <td class="w-min-50">
                      <select class="form-select consume_item select2-single"  name="consume_item[]" data-id="${staticIndex}" data-new="1" id="consume_item_${staticIndex}">
                                 <option value="">Select Item</option>      
                                   ${itemOptions}
   ...
   $(document).on("keyup change", ".consume_price, .consume_weight", function () {
      ...
      calculateConsumedTotals();
      //reCalculatePrice($(this).attr('data-id'));
   });
    function reCalculatePrice(id) {
      return; // skip for now
       let consume_item_id = $("#consume_item_"+id).val();
       let new_status = $("#consume_item_"+id).attr('data-new');
       let qty = $("#consume_weight_"+id).val();
       //alert(qty)
       let base_url = "{{ url('/') }}";
       let allRequests = [];
       let generatedData = [];
       $(".generated_weight").each(function () {
        ...
        $.when.apply($, allRequests).done(function () {
            generatedData.forEach(itemData => {
                let factor = (itemData.generatedWeight / itemData.perKg) || 0;
                if (!itemData.consumptionItems) return;
                itemData.consumptionItems.forEach(ci => {
                  //   let qty = ci.consumption_rate * factor;
                  //   qty = parseFloat(qty.toFixed(3));
                    if (ci.item_id != consume_item_id && new_status == 0) {
                     
                            return;
                        }
                    
                    // 🔍 check existing record
                    let existing = generated_items_quantity.find(row =>
                        row.generated_item_id == itemData.generatedItemId &&
                        row.consumed_item_id == ci.item_id
                    );
                    if (existing) {
                        
                        // ✅ update quantity
                        existing.quantity = parseFloat(
                            (parseFloat(existing.quantity) + parseFloat(qty)).toFixed(3)
                        );
                    } else {
                        
                        // ✅ push new record
                        generated_items_quantity.push({
                            generated_item_id: itemData.generatedItemId,
                            consumed_item_id: ci.item_id,
                            quantity: qty
                        });
                    }
                });

...
<div class="table-title-bottom-line d-flex justify-content-between align-items-center bg-plum-viloet shadow-sm py-2 px-4">
   <h5 class="m-0">
@if($type == 'raw')
List of Job Work Out – Raw Material
@else
List of Job Work Out – Finished Goods
@endif
</h5>

   <form method="GET"
action="{{ $type == 'raw' ? route('jobwork.out.raw') : route('jobwork.out.finished') }}">
      <div class="d-flex">
...
@if($type == 'raw')
<a href="{{ route('jobwork.create.raw') }}" class="btn btn-xs-primary">
   ADD
</a>
@else
<a href="{{ route('jobwork.create.finished') }}" class="btn btn-xs-primary">
   ADD
</a>
@endif
</div>
...
<tr>
   ...
   <td class="text-center">{{ $row->voucher_no_prefix }}</td>
   <td>{{ $row->account_name }}</td>
   <td class="text-end">{{ $row->total }}</td>
   <td class="text-center">
      @if($type == 'raw')
<a href="{{ route('jobwork.edit.raw', $row->id) }}">
@else
<a href="{{ route('jobwork.edit.finished', $row->id) }}">
@endif
...
<a title="View Job Work"
   href="{{ route('jobwork.view', ['type' => 'raw', 'id' => $row->id]) }}"
   target="_blank">
@else
<a title="View Job Work"
   href="{{ route('jobwork.view', ['type' => 'finished', 'id' => $row->id]) }}"
   target="_blank">
@endif
...
<div class="modal fade" id="delete_jobwork" tabindex="-1" aria-hidden="true">
   <div class="modal-dialog w-360 modal-dialog-centered">
      <div class="modal-content p-4 border-divider border-radius-8">
         ...
         <form method="POST" action="{{ route('jobwork.delete') }}">
            @csrf

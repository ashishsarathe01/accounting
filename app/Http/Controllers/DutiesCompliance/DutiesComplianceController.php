        ...
        if ($selectedGst != '') {
            ...
            // try {
                ...
                if (
                    ...
                    $check_return_compliances = DB::table('gst_return_compliances')
                                                    ->where('company_id',Session('user_company_id'))
                                                    ->where('gst_number',$selectedGst)
                                                    ->where('month_year',$monthKey)
                                                    ->where('return_type',$type)
                                                    ->first();
                                                    
                        if(!$check_return_compliances){
                            if($return['status']=="Filed"){
                                DB::table('gst_return_compliances')->insert([
                                    'company_id' => Session('user_company_id'),
                                    'return_type' => $type,
                                    'gst_number' => $selectedGst,
                                    'month_year' => $monthKey,
                                    'arn_number' =>  $return['arn'],
                                    'filed_date' => date('Y-m-d',strtotime($return['dof'])),

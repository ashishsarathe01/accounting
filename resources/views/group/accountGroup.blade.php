      ...
      <div class="row vh-100">
         ...
         <div class="col-md-12 ml-sm-auto  col-lg-10 px-md-4 bg-mint">
            ...
            <div class="position-relative table-title-bottom-line d-flex justify-content-between align-items-center bg-plum-viloet title-border-redius border-divider shadow-sm py-2 px-4">
               ...
               @can('action-module',71)
                  <a href="{{ route('account-group.create') }}" class="btn btn-xs-primary">
                     ADD
               ...
               <table id="example" class="table-striped table m-0 shadow-sm">
                  <thead>
                     <tr class=" font-12 text-body bg-light-pink ">
                        ...
                        <th class="w-min-120 border-none bg-light-pink text-body ">Heading </th>
                     ...
                     foreach ($accountgroup as $value) { ?>
                        <tr class="font-14 font-heading bg-white">
                           ...
                           <td class="w-min-120">
                              <?php 
                              if($value->heading!=''){
                                 if($value->heading_type=="group"){
                                    $accountgroup = DB::table('account_groups')->select('name')->where('id', '=', $value->heading)->first();
                                    echo $accountgroup->name;
                                 }else{
                                    $accountgroup = DB::table('account_headings')->select('name')->where('id', '=', $value->heading)->first();
                                    echo $accountgroup->name;
                                 }
                                 
                              } ?>
                           </td>
                           <!-- <td class="w-min-120"><?php echo $value->name_as_sch ?></td>
                           <td class="w-min-120">
                              <?php 
                              if($value->heading_as_sch!=''){
                                 $accountgroup = DB::table('account_headings')->select('name_sch_three')->where('id', '=', $value->heading_as_sch)->first();
                                 echo $accountgroup->name_sch_three;
                              } ?>
                           </td> -->
                           <td class="w-min-120">
                           ...
                           <td class="w-min-120 text-center">
                              ...
                              if($value->company_id!=0){?>
                                 @can('action-module',39)
                                    <a href="{{ URL::to('account-group/' . $value->id . '/edit') }}"><img src="{{ URL::asset('public/assets/imgs/edit-icon.svg')}}" class="px-1" alt=""></a>
                                 @endcan
                                 @can('action-module',40)
...
<div class="modal fade" id="delete_heading" tabindex="-1" aria-labelledby="exampleModalLabel" aria-hidden="true">
   <div class="modal-dialog w-360  modal-dialog-centered  ">
      <div class="modal-content p-4 border-divider border-radius-8">
         ...
         <form class="" method="POST" action="{{ route('account-group.delete') }}">
            @csrf

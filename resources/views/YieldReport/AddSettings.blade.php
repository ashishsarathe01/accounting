...
<form method="POST" action="{{ route('yield-report.store') }}">
@csrf
    ...
    <div id="material_container">
        ...
        <div class="row g-2 align-items-end material_row">
            ...
            <div class="col-md-4">
                ...
                <select name="material[0][item_id]" class="form-select select2-single item-select">
            ...
            <div class="col-md-3">
                ...
                <select name="material[0][recovery_status]" class="form-select recovery-select">
            ...
            <div class="col-md-3">
                <label class="form-label">Recovery %</label>
                <input type="text"
                    name="material[0][recovery_percent]"
...
<div class="card shadow-sm mt-4 p-4">
    ...
    <div id="main_material_container">
        ...
        <div class="row g-2 align-items-end main_material_row">
            ...
            <div class="col-md-10">
                <label class="form-label">Item</label>
                <select name="main_material[0][item_id]" class="form-select select2-single item-select">
                    <option value="" disabled selected>-- Select Item --</option>
                    @foreach($items as $item)
                        <option value="{{ $item->id }}">{{ $item->name }}</option>
                    @endforeach
...
$(document).on('change', '.item-select', function () {
    ...
    if (allSelected.includes(selectedValue)) {
        ...
        current.val('').trigger('change'); // reset select2 safely
    }
});
...
// When item changes
$(document).on('change', '.item-select', function () {
    setTimeout(refreshItemOptions, 100);
});

// After adding rows
$(document).on('click', '.addMaterial, .addMainMaterial', function () {
    setTimeout(refreshItemOptions, 200);
});
...
$(document).on('wheel', '.recovery-percent', function (e) {
    this.blur();   // remove focus so scroll doesn't change value
});
...
$(document).on('input', '.recovery-percent', function () {
    ...
    // ONLY limit max — don't destroy decimal typing

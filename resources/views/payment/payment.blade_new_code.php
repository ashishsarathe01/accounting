      ...
      <div class="row vh-100">
         ...
         <div class="col-md-12 ml-sm-auto  col-lg-10 px-md-4 bg-mint">
            ...
            <div class="table-title-bottom-line position-relative d-flex justify-content-between align-items-center bg-plum-viloet title-border-redius border-divider shadow-sm py-2 px-4">
               <h5 class="transaction-table-title m-0 py-2">List of Payment Voucher</h5>
               ...
               <table class="table-striped table m-0 shadow-sm payment_table">
                  <thead>
                     <tr class=" font-12 text-body bg-light-pink ">
                        ...
                        <th class="w-min-120 border-none bg-light-pink text-body ">Created By</th>
                        <th class="w-min-120 border-none bg-light-pink text-body ">Approved By</th>
                  ...
                  <tbody>
                     ...
                     foreach ($payment as $value) { 
                        $is_first = !in_array($value->pay_id, $shown_ids);
                     ?>
                        <tr class="font-14 font-heading bg-white">
                           ...
                           <td>
                              <?php if($is_first){ echo $value->created_by_name ?? '-'; } ?>
                           </td>
                           ...
                           <td>
                              ...
                              if($is_first){
                                 if($value->approved_status == 1){
                                    echo ($value->approved_by_name ?? '-') . '<br>';
                                    echo '<small>' . date('d-m-Y H:i', strtotime($value->approved_at)) . '</small>';
                                 } else {
                                    echo '<span>-</span>';
                                 }
                              }
...
<div class="modal fade" id="paymentDeleteModal" tabindex="-1" aria-labelledby="exampleModalLabel" aria-hidden="true">
   <div class="modal-dialog w-360  modal-dialog-centered  ">
      <div class="modal-content p-4 border-divider border-radius-8">
         <div class="modal-header border-0 p-0">
            <button type="button" class="btn-close" data-bs-dismiss="modal" aria-label="Close"></button>
         </div>
         <form class="" method="POST" action="{{ route('payment.delete') }}">
            @csrf
...
<script>
   ...
   $("#search").keyup(function () {
      ...
      $(".payment_table tr").each(function (index) {
         if (!index) return;

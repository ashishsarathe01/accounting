   ...
   <a href="javascript:void(0);" 
    ...
    <select class="form-control" name="credit_days" id="credit_days">
        ...
        @foreach($credit_days as $cd)
            <option value="{{ $cd->id }}">{{ $cd->days }} Days</option>
        @endforeach
    </select>
</div>
                  ...
                  <div class="mb-4 col-md-4 tcs_applicable_div common_div" style="display:none;">
                     <label class="form-label font-14 font-heading">TCS APPLICABLE</label>
                     <select class="form-select common_val" name="tcs_applicable" id="tcs_applicable">
                        <option value="0">No</option>
                        <option value="1"
                           @if(isset($id) && $account->tcs_applicable=='1') selected @endif>
                           Yes
                        </option>
                     </select>
                  </div>
...
$(document).ready(function() {
   ...
   if (window.hasGstinOnLoad) {
      lastVerifiedGstin = $("#gstin").val().trim();
   ...
   $("#gstin").on('input', function () {
      ...
      let currentGstin = $(this).val().trim();

      if (currentGstin !== lastVerifiedGstin) {
         resetGstFields();
      }
...
$("#verify_gstin").on('click', function () {

    let gstin = $("#gstin").val().trim();

    if (gstin.length !== 15) {
        alert("Enter valid 15 digit GSTIN");
        return;
    }
    ...
    $.ajax({
        ...
        success: function (data) {
            ...
            if (data.status == 1) {
                ...
                let matchedState = $('#state option[data-state_code="' + stateCode + '"]').val();

                if (matchedState) {
                    $("#state").val(matchedState).trigger('change');
                ...
                alert(data.message || "Invalid GSTIN");
                resetGstFields();
            }
        }
    });
   ...
   $("#under_group").change(function(){
      ...
      if(($(this).val()==1 && $('option:selected', this).attr('data-type')=='group') || ($('option:selected', this).attr('data-under_dutytaxes_status')=='1')){
         ...
         $(".account_no_div").show();
         $(".ifsc_code_div").show();
         // Show SMS status and credit day
         $(".sms_status_div").show();
   ...
   $(document).on('click', '.remove_address', function() {
    ...
    if (sessionCompanyId && sessionCompanyId !== formCompanyId) {
        ...
        const msg =
        `This Contra belongs to "${formCompanyName}"
